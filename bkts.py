...
class App(threading.Thread):
    ...
    def to_atelecom(self, method, url, payload=None, req_dict=None):
        ...
        try:
            ...
            logging.error("Connection ERROR" + msg)
            self.goto_payment_mode()
            return False

    def driver_logout(self):
        logging.debug("driver_logout")
        self._driver_on_route = False
        self.driver_card_number = None
    ...
    def get_trusted(self):
        ...
        if not r:
            ...
            sql = "INSERT OR IGNORE INTO trusted (`code`) VALUES (:code)"
            logging.debug("Trusted list: {}".format(str(trusted_list)))
            self.db_executemany(sql, trusted_list)
            '''
            for trusted in trusted_list:
                self.db_execute(sql, {'code': trusted})
            '''
            self.db().commit()
    ...
    def get_multypass(self):
        ...
        if not r:
            ...
            logging.debug("Multipass list: {}".format(str(multipass_list)))
            self.db_executemany(sql, multipass_list)
            '''
            for multipass in multipass_list:
                self.db_execute(sql, {'code': multipass})
            '''
            self.db().commit()
